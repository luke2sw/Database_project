...
if (isset($_COOKIE['username']) && isset($_COOKIE['codice_utente']) && isset($_COOKIE['ruolo'])) {
    ...
    try {
        // Query to get Medico Esterno information
        $queryMedicoEsterno = "SELECT * FROM MEDICOESTERNO WHERE CodiceFiscale = :username";
        $stmt = $conn->prepare($queryMedicoEsterno);
        $stmt->execute([':username' => $username]);
        $MedicoEsterno = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        if ($MedicoEsterno) {
            ...
            WHERE p.MedicoEsterno = :codice_utente
            ";
...
<body>
    <main style="margin-bottom:5%">
        ...
        <section>
            <div class="container" style="margin-top:2%">
                <!--Elenco Prescrizioni Effettuate-->
                        ...
                        <div class="card">
                            <div class="card-body">
                                <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                                    ...
                                    <!--Controlla se il paziente è già presente nel database-->
